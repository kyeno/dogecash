DogeCash Core version v5.4.0 is now available from: <https://github.com/dogecash/dogecash/releases>
...
If you are running an older version, shut it down. Wait until it has completely shut down, then run the installer (on Windows) or just copy over /Applications/DogeCash-Qt (on Mac) or dogecashd/dogecash-qt (on Linux).
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/dogecash-translations/), and every single person who tested the release candidates.
