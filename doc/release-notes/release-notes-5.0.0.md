DogeCash Core version *v5.0.0* is now available from:  <https://github.com/dogecash/dogecash/releases>
...
DogeCash Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
See the release notes from [v4.2.0](https://github.com/dogecash/DogeCash/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getaccountaddress` | Returned the current DogeCash address associated with an internal account | Account system removal |
| `setaccount` | Associate a DogeCash address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given DogeCash address | Account system removal |
| `getaddressesbyaccount` | Returned a list of DogeCash addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a DogeCash address | Account system removal |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_dogecash unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake dogecashd and unit tests: fixing missing MacOS framework. (furszy)
