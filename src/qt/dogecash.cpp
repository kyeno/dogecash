...
{
    window = new DogeCashGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &DogeCashGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &DogeCashGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &DogeCashGUI::handlePaymentRequest);
        connect(window, &DogeCashGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. DogeCash can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
