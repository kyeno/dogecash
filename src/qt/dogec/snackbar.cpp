...
#include "qt/dogec/forms/ui_snackbar.h"
#include "qt/dogec/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(DogeCashGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &DogeCashGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
