...
SettingsFaqWidget::SettingsFaqWidget(DogeCashGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    QString unspendableDOGECContent = formatFAQContent(
        formatFAQParagraph(
    ...
    ui->labelContent_UnspendableDOGEC->setText(unspendableDOGECContent);
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("dogecash-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
        ...
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::DOGEC)) +
